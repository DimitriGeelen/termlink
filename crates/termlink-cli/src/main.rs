...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::List { all, json, tag, name, role, cap, count, names, no_header } => commands::session::cmd_list(all, json, tag.as_deref(), name.as_deref(), role.as_deref(), cap.as_deref(), count, names, no_header),
        ...
        Command::Event(ev) => match ev {
            ...
            EventCommand::Watch { targets, interval, topic, json, timeout, count } => {
                commands::events::cmd_watch(targets, interval, topic.as_deref(), json, timeout, count).await
            }
            ...
            EventCommand::Collect { targets, topic, interval, count, json, timeout } => {
                commands::events::cmd_collect(targets, topic.as_deref(), interval, count, json, timeout).await
            }
        },
        ...
        Command::Watch { targets, interval, topic, json, timeout, count } => {
            commands::events::cmd_watch(targets, interval, topic.as_deref(), json, timeout, count).await
        }
        Command::Topics { target, json, timeout } => commands::events::cmd_topics(target.as_deref(), json, timeout).await,
        Command::Collect { targets, topic, interval, count, json, timeout } => {
            commands::events::cmd_collect(targets, topic.as_deref(), interval, count, json, timeout).await
        }

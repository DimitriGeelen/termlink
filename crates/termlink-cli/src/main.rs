...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::List { all, json, tag, name, role, cap, count, names, ids, first, no_header } => commands::session::cmd_list(all, json, tag.as_deref(), name.as_deref(), role.as_deref(), cap.as_deref(), count, names, ids, first, no_header),

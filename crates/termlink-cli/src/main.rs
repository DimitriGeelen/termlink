...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Event(ev) => match ev {
            ...
            EventCommand::Watch { targets, interval, topic, json, timeout } => {
                commands::events::cmd_watch(targets, interval, topic.as_deref(), json, timeout).await
            }
        ...
        Command::Watch { targets, interval, topic, json, timeout } => {
            commands::events::cmd_watch(targets, interval, topic.as_deref(), json, timeout).await
        }

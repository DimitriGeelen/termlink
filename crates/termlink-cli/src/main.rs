...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::List { all, json, tag, name, role, count } => commands::session::cmd_list(all, json, tag.as_deref(), name.as_deref(), role.as_deref(), count),
        ...
        Command::Event(ev) => match ev {
            ...
            EventCommand::Collect { targets, topic, interval, count, json } => {
                commands::events::cmd_collect(targets, topic.as_deref(), interval, count, json).await
            }
        },
        ...
        Command::Collect { targets, topic, interval, count, json } => {
            commands::events::cmd_collect(targets, topic.as_deref(), interval, count, json).await
        }
        ...
        Command::Discover { tag, role, cap, name, json, count, first, wait, wait_timeout } => {
            commands::metadata::cmd_discover(tag, role, cap, name, json, count, first, wait, wait_timeout).await
        }
        Command::Kv { target, json, timeout, raw, action } => commands::metadata::cmd_kv(&target, action, json, raw, timeout).await,

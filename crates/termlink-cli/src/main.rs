...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Agent { action } => match action {
            ...
            AgentAction::Contact { target, target_fp, message, thread, hub, json, dry_run, require_online, online_window_secs, ack_required, ack_timeout_secs } => {
                commands::agent::cmd_agent_contact(target.as_deref(), target_fp.as_deref(), &message, thread.as_deref(), hub.as_deref(), json, dry_run, require_online, online_window_secs, ack_required, ack_timeout_secs).await
            }
            ...
            AgentAction::Recent { target, target_fp, n, window_secs, filter_thread, filter_project, filter_msg_types, hub, json, watch, watch_interval } => {
                let mt: Vec<&str> = filter_msg_types.iter().map(String::as_str).collect();
                let mt_opt = if mt.is_empty() { None } else { Some(mt.as_slice()) };
                commands::agent::cmd_agent_recent(target.as_deref(), target_fp.as_deref(), n, window_secs, filter_thread.as_deref(), filter_project.as_deref(), mt_opt, hub.as_deref(), json, watch, watch_interval).await
            }
            AgentAction::OnThread { thread, n, window_secs, filter_project, filter_msg_types, peer, peer_fp, hub, json, watch, watch_interval } => {
                let mt: Vec<&str> = filter_msg_types.iter().map(String::as_str).collect();
                let mt_opt = if mt.is_empty() { None } else { Some(mt.as_slice()) };
                commands::agent::cmd_agent_on_thread(&thread, n, window_secs, filter_project.as_deref(), mt_opt, peer.as_deref(), peer_fp.as_deref(), hub.as_deref(), json, watch, watch_interval).await
            }
            ...
            AgentAction::Timeline { n, window_secs, filter_thread, filter_project, filter_msg_types, hub, json, watch, watch_interval } => {
                let mt: Vec<&str> = filter_msg_types.iter().map(String::as_str).collect();
                let mt_opt = if mt.is_empty() { None } else { Some(mt.as_slice()) };
                commands::agent::cmd_agent_timeline(n, window_secs, filter_thread.as_deref(), filter_project.as_deref(), mt_opt, hub.as_deref(), json, watch, watch_interval).await
            }
        },
        ...
        Command::Fleet { action } => match action.unwrap_or(FleetAction::Status { json: false, timeout: 10, verbose: false }) {
            ...
            FleetAction::Doctor { json, timeout, legacy_usage, legacy_window_days, topic_durability, diff, save_snapshot, exit_code_on_verdict, trend, trend_keep, top_callers } => {
                commands::remote::cmd_fleet_doctor(json, timeout, legacy_usage, legacy_window_days, topic_durability, diff, save_snapshot, exit_code_on_verdict, trend, trend_keep, top_callers).await
            }

...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Remote { action } => match action {
            ...
            RemoteAction::List { hub, secret_file, secret, scope, name, tags, roles, cap, json } => {
                let p = resolve_hub_profile(&hub, secret_file.as_deref(), secret.as_deref(), &scope)?;
                commands::remote::cmd_remote_list(&p.address, p.secret_file.as_deref(), p.secret.as_deref(), p.scope.as_deref().unwrap_or("observe"), name.as_deref(), tags.as_deref(), roles.as_deref(), cap.as_deref(), json).await
            }

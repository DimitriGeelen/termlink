...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Discover { tag, role, cap, name, json, count, first, wait, wait_timeout, id } => {
            commands::metadata::cmd_discover(tag, role, cap, name, json, count, first, wait, wait_timeout, id).await
        }

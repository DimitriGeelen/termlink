...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Fleet { action } => match action.unwrap_or(FleetAction::Status { json: false, timeout: 10, verbose: false }) {
            ...
            FleetAction::Doctor { json, timeout, legacy_usage, legacy_window_days, topic_durability, diff, save_snapshot, exit_code_on_verdict, trend, trend_keep } => {
                commands::remote::cmd_fleet_doctor(json, timeout, legacy_usage, legacy_window_days, topic_durability, diff, save_snapshot, exit_code_on_verdict, trend, trend_keep).await
            }

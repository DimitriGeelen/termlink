...
/// cheaper than introducing a cross-module dependency).
///
...
{\"ts\":\"2030-01-01T00:02:00Z\",\"agent_id\":\"d\",\"kind\":\"removed\"}\n";
    ...
    #[test]
    fn find_idle_history_parse_counts_unparseable_ts_as_malformed() {
        ...
        // Before the fix, rfc3339_to_unix_secs_local returned the 0-sentinel for
        // "not-a-date", 0 < cutoff was always true, and the row vanished WITHOUT
        // incrementing `malformed` — the caller then reported "0 malformed lines
        // skipped" while a corrupt row disappeared.
        // Temp-revert proof: restore the `-> i64` 0-sentinel helper + the
        // `let entry_secs = rfc3339_to_unix_secs_local(ts_str)` call site and this
        // test FAILS on `malformed == 1` (it reads 0).
        let text = "{\"ts\":\"not-a-date\",\"agent_id\":\"a\",\"kind\":\"new\"}\n";
        let cutoff = 1_000_000i64; // any positive cutoff exposes the 0-sentinel drop
        let (entries, malformed) = parse_find_idle_log(text, cutoff, None, None);
        ...
        let old = "{\"ts\":\"1970-01-05T00:00:00Z\",\"agent_id\":\"a\",\"kind\":\"new\"}\n";
        let (entries, malformed) = parse_find_idle_log(old, cutoff, None, None);
...
{\"ts\":\"2030-01-01T00:00:00Z\",\"agent_id\":\"fresh\",\"kind\":\"new\"}\n";
        ...
        let cutoff = rfc3339_to_unix_secs_local("2027-01-01T00:00:00Z").unwrap();

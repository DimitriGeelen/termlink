...
- **Agent:** Mode B is the answer — hard-blocking would actually lose information. Re-stated goal as "no information loss" and enumerated the 5 prerequisites (capture, storage, pointer, discoverability, threshold). 4 of 5 are already in place via `fw bus`. The one unknown is whether SubagentStop can mutate the orchestrator-visible message — that's S1'. Pending: live migration from day 1 (8KB) or passive size survey first?
- **Human (2026-04-24):** Option B — live from day 1.
- **Agent:** Locked in. Initial threshold T=8KB; over-threshold returns auto-migrate to `fw bus`. S1' still gates the *shape* of the agent-visible result (mutated response vs stderr-nudge-to-read-bus), but EITHER outcome preserves information. S2 (size survey) still runs in parallel as logging — informs whether 8KB is right or should be tuned to 4KB / 16KB after a week of real data.

# PROMPT — Project Value Review (delete, refactor, add)

Parameters for this run (filled by orchestrator T-3044; recorded so the fill is auditable):
- {{SCOPE}}           = whole repo (/opt/termlink)
- {{PURPOSE_SOURCE}}  = docs/CHARTER.md (authoritative, human-blessed) + policy/value-drivers.yaml
- {{EXTERNAL_DATA}}   = none
- {{BUDGET}}          = 200k tokens
...
======================================================================
  ...
  Hotspots = churn x complexity  what costs every time it's touched      R
...
--- Workflow Designer / workflows -----------------------------------
  Ratified workflows              the determinism map: which processes are D (protect)
   (BPMN XML with aef: ext.)      defined and ratified. Anything a
...
--- REFACTOR ------------------------------------------------------
  Signals for: hotspot (churn x complexity); high fix/revert or rework

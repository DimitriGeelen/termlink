...
- **Slice 12 (T-2072):** `channel claims-summary [<TOPIC>|--all] --watch <secs> --notify <CMD>` event hook — turns the watch loop from a human-visible dashboard into an automation source. On per-topic stuck-state transitions (and `new`/`removed` topics under `--all`) fires `<CMD>` fire-and-forget via `sh -c` with env vars: `TERMLINK_CLAIM_TOPIC`, `TERMLINK_CLAIM_CHANGE_KIND` (`transition`/`new`/`removed`), `TERMLINK_CLAIM_TS` (RFC3339), `TERMLINK_CLAIM_HUB`, `TERMLINK_CLAIM_OLD_STUCK` / `TERMLINK_CLAIM_NEW_STUCK` (`true`/`false`/`n/a`), `TERMLINK_CLAIM_ACTIVE_COUNT`, `TERMLINK_CLAIM_EXPIRED_COUNT`, `TERMLINK_CLAIM_OLDEST_AGE_MS` (or `n/a`). Baseline tick (no prior state) fires zero events. Hanging scripts do NOT block the loop; command-not-found prints a one-line warning and the watch continues. Pure helper `diff_claim_states` is the unit-tested heart of the diff (mirror of T-2065's governor `diff_*` helper). Common gate in the script: `[ "$TERMLINK_CLAIM_NEW_STUCK" = "true" ] || exit 0` then page. Recipe: `termlink channel claims-summary --all --watch 30 --notify /usr/local/bin/page-on-stuck.sh`.

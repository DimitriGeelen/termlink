...
- [ ] Registry resolves `agent_id` → `{host:port, hub, topics-read, liveness}` for a registered peer
- [ ] Observed address is hub-stamped (hub records the source addr it saw), with self-report as fallback when unavailable
- [ ] A fleet-rollup reader walks `hubs.toml` and merges per-hub registries with a short TTL
- [ ] Reverse lookup works: a recipient can resolve the SENDER's address (symmetric, both directions)
- [ ] The G-155 false-green probe is fixed to test the intended correspondent, not just a configured peer

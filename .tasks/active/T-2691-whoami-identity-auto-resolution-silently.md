...
description: >
  read_ppid_from_proc reads /proc/<pid>/stat with .ok()? on both the CLI (metadata.rs)
  and MCP (tools.rs whoami_helpers) surfaces. On macOS the ancestor chain collapses

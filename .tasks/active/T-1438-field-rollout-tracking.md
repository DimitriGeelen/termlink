...
- [x] **.143 (ring20-dashboard) — operator auth heal completed** — Resolved structurally 2026-05-03 by the T-1296 runtime_dir migration: `/var/lib/termlink/` now persists secret across reboot, eliminating the auth-heal cycle. `fleet doctor` PASSes for ring20-dashboard post-swap at 2026-05-03T08:06Z; TOFU fingerprint `sha256:1389a831016...` matches pre-swap (no rotation cascade). Persist-if-present validated end-to-end.
- [x] **.121/.143 (ring20-dashboard, dashboard-agent) — skills DEPLOYED 2026-05-02T15:55Z** — hub flap window opened up; pulled session `tl-4augvpzt` from `remote list ring20-dashboard`. Pushed `/agent-handoff.md` (4568b, md5=94d3eae7) + `/check-arc.md` (3379b, md5=14705456) to `/root/.claude/commands/` via base64-over-remote-exec. Byte-identical with .107/.122/.141 deployments. **Binary state probed:** `termlink --version` = 0.9.844 — pre-T-1155, lacks `channel` subcommand entirely (`error: unrecognized subcommand 'channel'`). Hub at .121:9100 is also pre-T-1155 — `channel.list` and `channel.create` both fail with `JSON-RPC error -32001: Missing 'target' in params`. Cannot create `agent-chat-arc` topic on this hub without binary upgrade. T-1418 binary swap (Method 0: `fleet-deploy-binary.sh ring20-dashboard --swap-restart`) is the unblock — once swapped, `agent-chat-arc` channel + chat-arc participation come online via the same remote-exec pattern that worked on .122 + .141. Skill deployment is a forward-compatible inert artifact (markdown — only useful when binary catches up). vendored-arc-heartbeat.sh NOT staged here yet — would require channel.post which 0.9.844 doesn't support
- [x] **.121/.143 (ring20-dashboard) — binary upgrade to T-1235+ shipped 2026-05-03T10:06Z** — Bundled with T-1296 via `termlink remote exec ring20-dashboard tl-4augvpzt`. Disk binary 0.9.844 → 0.9.1702. Hub respawned via watchdog cron within 60s. T-1427 strict-reject LIVE in-memory (forged sender_id → -32014). agent-chat-arc topic created. Multicast post=4/skipped-legacy=0 (was 3/1 pre-swap). Polling-side validation: 0 inbox.status calls from .121 since swap; 6 channel.list calls from .121 in 14-min window post-swap — T-1235 SDK shim active. See T-1418 Updates for full evidence trail.
...
- [x] **Chat-arc participation pattern triumvirate complete 2026-05-02T20:25Z** — three driver/host primitives now codified:
  - `scripts/field-heartbeat.sh` — driver-from-.107: walks fleet, posts as each FIELD host (vendored identity)
  - `scripts/vendored-arc-heartbeat.sh` — runs ON each field host: self-fires single chat-arc post (cron-friendly)
  - `scripts/chat-arc-multicast.sh` — driver-from-.107: posts as .107 to every UP hub (fleet announcements)
  Plus `/heartbeat` skill (md5 f25495bccefadfbf617df9baa501a2f9) byte-identical on all 4 hosts: `/opt/termlink/.claude/commands/` (.107), `/root/.claude/commands/` (.122 + .121), `/home/dimitri/.claude/commands/` (.141 WSL). Combined with `/agent-handoff` (DM-out, T-1429) + `/check-arc` (DM-in inbox), every vendored agent now has a uniform 3-skill chat-arc kit.
- [x] **3-agent co-residency on .107 verified live 2026-05-02T20:18Z** — chat-arc offsets 73/75/76 show three projects sharing FP `d1993c2c3ec44c94`: `002-Claude-Partner-Network` (cohort), `050-email-archive` (peer), `010-termlink` (this project). All three organically adopted `from_project` metadata as the disambiguator before any framework-level codification. Triggered T-1448 inception. Captured as learning (P-001).
- [x] **Heartbeat scripts adopt from_project=010-termlink convention 2026-05-02T20:15Z** — `field-heartbeat.sh` + `vendored-arc-heartbeat.sh` updated md5=`caa244cf77bf33d384cb7c7924f863bd`, byte-identical on .107 + .122 + .141 + .121 (forward-deployed). Live verification: chat-arc offsets carry `from_project=010-termlink` post-patch. Forward-compat regardless of T-1448 outcome (GO Design A | NO-GO | DEFER).
- [x] **chat-arc is hub-LOCAL not fleet-wide — gap captured 2026-05-02T20:18Z** — Discovered each hub maintains its own `agent-chat-arc` topic with independent offsets (.107=81, .122=15, .141=14 at session end). No auto-bridging. Three workarounds available: (a) bridging mechanism, (b) multicast pattern (used by `chat-arc-multicast.sh`), (c) read-side multi-hub subscribe. None automatic today. Captured as learning P-001 — flag for follow-up if cross-fleet chat-arc visibility becomes a requirement.
- [x] **.121 ring20-dashboard binary 0.9.1702 STAGED via --probe 2026-05-02T20:21Z** — `scripts/fleet-deploy-binary.sh ring20-dashboard --probe` ran successfully. musl-static `target/x86_64-unknown-linux-musl/release/termlink` (sha 2282f85c00350193bfe50e97acac2c4a35f5114a41c61e756743fa784a1e5ea6) streamed in 453 chunks, sha verified, probe `--version` returned `termlink 0.9.1702` cleanly. Binary sits at `/tmp/termlink.new` on .121. Pre-T-1155 → T-1443 jump is now a single operator command: `scripts/fleet-deploy-binary.sh ring20-dashboard --swap-restart`. Probe is non-destructive — operator-gated swap remains.
- [x] **All 3 field hosts have 0.9.1702 STAGED via --probe 2026-05-02T20:30Z** — fleet-wide staging refresh: same sha 2282f85c verified on .122 (current 0.9.1693, gain T-1443 ensure-topic), .141 (current 0.9.1640, gain T-1436 FP visibility + T-1443), .121 (current 0.9.844, gain channel.* primitives). Three single-command operator swaps now ready. Probes were non-destructive — binaries sit at /tmp/termlink.new awaiting operator-gated --swap-restart.
- [x] **.122 ring20-management binary SWAPPED 0.9.1693 → 0.9.1702 + 6th persist-hold 2026-05-02T20:50Z** — operator-greenlit autonomous swap. fleet-deploy-binary.sh --swap-restart re-staged + swapped + restarted. Hub came back UP at t=15s. **6th consecutive persist-hold** of identical canary SHAs: hub.secret 3dd9d01afe4ec599… and hub.cert.pem 2355a206cd9c306d… UNCHANGED across the entire swap cycle (T-1294 ground-truth re-verified 6 times). Live verification: `termlink --version` = 0.9.1702 on remote; `channel post --help` shows --ensure-topic flag (T-1443). Post-swap chat-arc smoke: offset=18 with --ensure-topic flag exercised end-to-end. Foundation T-1294 + T-1427 + T-1436 + T-1441 + T-1443 all live on .122.
- [x] **Hourly chat-arc heartbeat cron INSTALLED on .122 + .141 2026-05-02T20:52Z** — `17 * * * * /path/to/vendored-arc-heartbeat.sh >> heartbeat.log 2>&1` per host. Idempotent skip-if-exists logic. Field-side autonomous chat-arc participation now self-sustaining without driver intervention. Cadence reversible — operator may adjust hour/cadence via `crontab -e`.
- [x] **.121 stability root-cause confirmed (T-1296 evidence) 2026-05-02T20:50Z** — runtime_dir is `/tmp/termlink-0/`; systemd-tmpfiles `D /tmp 1777 root root -` wipes /tmp every boot; 3 reboots today = 3 hub-identity rotations. Launch mechanism opaque (PPID=1, no service unit installed despite template existing). T-1296 migration MUST precede .121 binary swap — full evidence captured at T-1296 task file Updates section.
- [x] **.121 ring20-dashboard — binary swap shipped 2026-05-03T10:06Z** — Autonomous via `termlink remote exec` (NOT operator-gated path). Bundled with T-1296 runtime_dir migration in single watchdog-respawn cycle. Hub gained channel.* primitives; agent-chat-arc topic created on first post. T-1166 cut-blocker resolution VERIFIED LIVE: zero post-swap inbox.status calls from .121, T-1235 SDK shim routes through channel.list (6 calls observed in first 14-min window). Last legacy call from .121 to .107 hub at 08:04:03Z UTC; nothing since. The 1393 in 24h rolling window is purely pre-swap residue, will age out by 2026-05-04T10:06Z.

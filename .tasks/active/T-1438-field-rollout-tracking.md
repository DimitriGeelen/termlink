...
last_update: 2026-05-02T11:49:39Z
date_finished: null
---
...
- [ ] **.121/.143 (ring20-dashboard) — binary upgrade to T-1235+ shipped** — T-1418 dependency, operator-gated. Method 0: `./scripts/fleet-deploy-binary.sh ring20-dashboard --swap-restart`. Pre-req: hub stable (not flapping). Post-swap: re-run skill-byte verification, deploy vendored-arc-heartbeat.sh, smoke-test chat-arc post
...
- [ ] **.121 ring20-dashboard — binary swap shipped (operator-gated)** — `scripts/fleet-deploy-binary.sh ring20-dashboard --swap-restart`. Pre-req: hub stable (no flap). Post-swap: hub gains `channel.*` primitives; agent-chat-arc topic creatable; T-1166 cut blocker partially clears (1400/1401 legacy calls/24h originate from .121 polling-client per T-1419 freshness signal).

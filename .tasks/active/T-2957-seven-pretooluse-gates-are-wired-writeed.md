---
id: T-2957
name: "Seven PreToolUse gates are wired Write|Edit only, so Bash heredoc task edits
  bypass them"
description: >
  check-active-task matches Write|Edit|Bash but check-human-ac-tick, check-active-completed-dup,
  check-arc-id, check-heredoc-cmd-sub, check-inception-decisions, check-inception-schema
  and check-onboarding-gate match Write|Edit only. A task file rewritten from a Bash
  heredoc ticks Human ACs with no block and no Tier-2 log. Reported by 050-email-archive
  (P-EMAIL-ARCHIVE-001), verified live here.

...
description: >
  check_protocol_version() exists in control.rs with a passing unit test and ZERO
  callers, so PROTOCOL_VERSION_TOO_OLD can never be returned and the hub cannot refuse
  an out-of-date peer. Wiring it starts rejecting live peers below the required version
  — which methods gate on which version is a product decision with fleet blast radius
  (T-2698 G3).

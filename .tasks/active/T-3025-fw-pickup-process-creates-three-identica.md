---
id: T-3025
name: "fw pickup process creates three identical tasks per envelope, two of them ~47s later and unannounced"
description: >
  arc-008 run finding. Each fw pickup process run created THREE identical local tasks from ONE envelope: P-079 -> T-3019/T-3020/T-3021 (13:20:02/02/03), P-080 -> T-3022/T-3023/T-3024 (13:30:15, 13:31:02/02). The console names only the first; the other two appear ~47s later, so a check run immediately after process sees one task and reads clean. Filing two reports upstream injected six spurious tasks into .tasks/active/ - 3x amplification per filing. Also observed: metadata.from_project is inconsistent across the two filings (offset 126 'root', offset 127 '010-termlink') through the identical path, and T-2816's canary self-filter keys on that field so an own-filing will fire the framework-pickup canary. Vendored pipeline (lib/pickup.sh, lib/pickup-channel-bridge.sh) so the fix is upstream per G-062. Linked to T-3015/T-3016, not merged - arc-008 rule.
...
last_update: 2026-09-20T13:35:39Z
date_finished: null
...
- [ ] The mechanism is established rather than guessed: show that the duplicate tasks are
...
- [ ] The dedup failure is root-caused in `lib/pickup.sh`: `pickup_dedup_hash` reports
      `envelope not readable` against the INBOX path after the envelope has already been moved
      to `processed/`, so `pickup_record_dedup` refuses to record — leaving nothing to suppress
      the next cron pass. Cite the ordering with line numbers.
- [ ] The six spurious tasks from this run (T-3019/T-3020/T-3021, T-3022/T-3023/T-3024) are
      dispositioned — closed as duplicates or otherwise resolved, with the disposition recorded.
- [ ] Filed upstream to `framework:pickup` per G-062 with the mechanism, the ordering defect and
...
### 2026-09-20T13:35:39Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/termlink/.tasks/active/T-3025-fw-pickup-process-creates-three-identica.md

...
description: >
  Self-contained shell demo: N synthetic workers drain an M-unit work-queue on the
  live hub via the claim primitive, asserting each unit is won by exactly one worker
  (no double-claim) — the operator-facing proof of arc-001's headline mechanic. Composes
  existing verbs (channel create/post/claim/claims-summary); no new primitive, no
  human-gated inception. Captures demo_evidence for the arc.

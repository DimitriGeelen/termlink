---
id: T-2850
name: "Land the 7 guards stranded on the charter-review worktree branch"
description: >
...
last_update: 2026-08-29T09:37:35Z
date_finished: null
...
# Every landed guard must be tracked, not merely present on disk.
test -n "$(git ls-files scripts/check-strict-star.sh)"
test -n "$(git ls-files scripts/check-unbounded-rpc-call.sh)"
test -n "$(git ls-files scripts/check-error-swallowing-predicate.sh)"
test -n "$(git ls-files scripts/check-task-template-idioms.sh)"
test -n "$(git ls-files scripts/check-hook-counter-integrity.sh)"
test -n "$(git ls-files scripts/check-addressed-posts.sh)"
test -n "$(git ls-files scripts/check-dashboard-deleted-root.sh)"
# Their allowlists must be tracked too, or every acknowledged site fires in a
# fresh clone while reporting clean here — the exact T-2681 defect.
test -n "$(git ls-files .context/checks/strict-star-allowlist)"
test -n "$(git ls-files .context/checks/unbounded-rpc-call-allowlist)"
...
### 2026-08-29T09:37:35Z — task-created [task-create-agent]

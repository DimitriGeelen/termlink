...
last_update: 2026-03-10T08:44:46Z
date_finished: null
---
...
### Agent
- [ ] Shared `setup.sh` exists with common orchestrator registration, health check, and binary resolution
- [ ] All 6 e2e level scripts source `setup.sh` instead of duplicating setup logic
- [ ] No hardcoded absolute paths (e.g., `/Users/dimidev32/...`) in any e2e test script
- [ ] Binary paths resolved via `$PATH`, `which`, or `TERMLINK_BIN`/`CLAUDE_BIN` env vars
...
grep -q 'source.*setup.sh\|\. .*setup.sh' tests/e2e/level-1-*.sh

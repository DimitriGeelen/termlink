...
description: >
  Execute the T-2454 GO decision: all 5 claim verbs (claim/renew/release/transfer/force-release) enforce ownership against a caller-supplied free-text claimer/by string with NO cryptographic identity binding, while channel.post correctly binds identity via T-1427 (sender_id==fingerprint(pubkey)). Any Interact-scoped peer can release/renew/transfer another agent's claim → double-grant / silent work-loss. Fix: apply the T-1427 signature pattern to claim params (protocol change across hub+session+cli+mcp). GO-to-build is the human's per T-2454.
...
last_update: 2026-08-08T07:59:23Z
date_finished: null

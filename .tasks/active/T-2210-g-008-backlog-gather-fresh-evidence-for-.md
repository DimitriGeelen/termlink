...
description: >
  For the ~48 partial-complete tasks, run the mechanically-verifiable Human-AC verification
  steps, capture fresh evidence, and inject timestamped Updates entries so the human
  can batch-confirm. Does NOT tick Human ACs (sovereignty). Surfaces operator-bound
  ACs separately. Batch-injection per G-008 pattern.

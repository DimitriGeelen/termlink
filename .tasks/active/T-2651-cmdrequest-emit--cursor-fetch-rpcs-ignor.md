---
id: T-2651
name: "cmd_request emit + cursor-fetch RPCs ignore --timeout (naked rpc_call, no wall-clock bound)"
description: >
  execution.rs cmd_request uses naked client::rpc_call for the initial event.subscribe cursor-fetch (line 182) and event.emit (line 197). Both call Client::call which reads via unbounded next_line() with no timeout. The user's --timeout only governs the subscribe wait loop, so a wedged target session hangs cmd_request forever despite --timeout. Hardened sibling rpc_call_addr_with_timeout (T-2354) exists but was not used here.
...
last_update: 2026-08-12T19:14:20Z
date_finished: null

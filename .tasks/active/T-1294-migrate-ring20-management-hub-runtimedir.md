...
- **AC 4 (re-pin client + fleet doctor):** In progress. Local /opt/termlink (.102) still has stale cache → `termlink fleet doctor` failing with `invalid signature`. Next: read live secret from `/var/lib/termlink/hub.secret` on .122 console, heal locally.

...
MCP arc cycle 10 slice 2, follow-on to T-1970. `tool_detail` already returns `parameters: [{name, type, optional, doc}]` — adding a `parameter_count` integer alongside lets the LLM rank tool complexity in O(1) without walking the array. Useful for cold-start cost-aware routing.
...
### 2026-06-04T05:48:32Z — task-created [task-create-agent]

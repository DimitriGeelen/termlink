...
description: >
  check-cron-install-drift.sh fires UNINSTALLED_JOBS on agentic-audit.crontab: both
  git-declared flock-guarded 'fw pickup process' job lines are absent from /etc/cron.d/agentic-audit-termlink,
  and a second unguarded per-minute copy runs from /etc/cron.d/agentic-pickup-termlink.
  Measured contributing cause of the T-2953 double-mint (one envelope minted twice,
  60s apart, twice in one day). Needs root to install.

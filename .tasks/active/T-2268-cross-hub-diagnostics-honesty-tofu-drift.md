...
last_update: 2026-06-24T07:55:23Z
date_finished: null
...
- [ ] A TOFU/cert-drift connect failure surfaces the "TOFU VIOLATION … run `termlink tofu clear`" remediation verbatim, NOT the generic "is the hub running?" message — verified by a test that injects a drifted pin and asserts the surfaced string.
- [ ] On a remote `-32601 Method not found`, the caller annotates whether it is likely version-skew (remote hub lacks the method) vs a typo — using a remote `hub.version`/capabilities probe and/or surfacing `-32011` data fields when present.
- [ ] `-32010` rendering distinguishes insufficient-scope (echoes required vs granted + the token-create hint) from bad/stale credential — caller can tell auth from authz from the message alone.
- [ ] `cargo test -p termlink-mcp -p termlink-session` passes; `cargo build` clean.

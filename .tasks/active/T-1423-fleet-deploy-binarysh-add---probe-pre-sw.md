...
description: >
  PL-100 incident on .122 (T-1422) showed the script will happily kill+swap a binary
  that the target host can't actually execute (glibc/lib mismatch between build host
  and target). Add --probe: before kill, run NEW_BIN --version with a short timeout
  on the remote; abort if non-zero. Cheap pre-flight that would have caught this with

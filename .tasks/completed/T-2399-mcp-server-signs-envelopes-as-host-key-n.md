---
id: T-2399
name: "MCP server signs envelopes as host key not per-agent identity (outbound identity
  leak)"
description: >
  MCP server signs envelopes as host key not per-agent identity (outbound identity
  leak)

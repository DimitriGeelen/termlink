---
id: T-2029
name: "Substrate (T-2019 Slice 1): claims SQLite table + channel.claim + channel.release
  RPCs"
description: >
  Slice 1 of 3 from T-2019 GO. Schema migration: add claims table to bus/meta.rs init_schema.
  Two new RPC verbs: channel.claim (params: topic, offset, lease_secs, claimed_by
  → claim_id, claimed_until; INSERT-OR-FAIL on UNIQUE(topic, offset)) and channel.release
  (params: claim_id, ack: bool → ok; on ack=true advance claiming subscriber cursor
  past released offset). Method constants in termlink-protocol/src/control.rs::method;
...
# T-2029: Substrate (T-2019 Slice 1): claims SQLite table + channel.claim + channel.release RPCs
...
grep -q "CREATE TABLE IF NOT EXISTS claims" crates/termlink-bus/src/meta.rs
...
### 2026-06-07T12:12:11Z — task-created [task-create-agent]
...
### 2026-06-07T22:03:51Z — status-update [task-update-agent]

...
description: >
  Deploy follow-up: the installed /root/.cargo/bin/termlink is 0.11.296 (pre-fix);
  main now carries T-2352 (self-fp chain + peer-posted thread preference), T-2353
  (agent-send --hub), T-2354 (bounded TCP RPC reads). CLI-only change — NO hub restart
  needed (client.rs/channel.rs are client-side). Path: cargo build --release -p termlink,
  backup existing binary, rm-then-cp (avoids ETXTBSY), verify termlink --version >=
  0.11.313 and the T-2354 seam (TERMLINK_RPC_READ_TIMEOUT_SECS bounded error vs wedged
  .122 read). Closes the preflight Check 4 stale-binary WARN.

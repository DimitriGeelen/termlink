...
description: >
  MCP hub_probe emits {ok, address, fingerprint, error}; CLI emits {address, fingerprint}
  on success and non-JSON on failure. Add ok+error to CLI, structure CLI failure path
  as JSON, add parity_hub_probe test.

...
description: >
  Three mitigations per G-016: (1) add MAX_RECOVERIES per-invocation cap (default
  10) to session-silent-scanner.sh inner loop, (2) add SESSION_SILENT_MAX_AGE_DAYS
  ceiling (default 7) to filter out ancient agent-acompact-* sessions, (3) optional:
  30s rate limit between fw handover invocations. Also: trace root cause of DRY_RUN=0
  in the 13:38 bootstrap that triggered the 92-commit storm — if shell snapshot carries
  it, every claude session re-triggers.
...
bvp_scores_proposed:
  - ts: '2026-08-18T18:55:50Z'
    ...
    rationale: D1=2 (body:concern-ref); D2=0 (no-signal); D3=0 (no-signal); D4=0
      (no-signal); F-RECALL=1 (body:episodic-only); F-ORCH=1 

...
description: >
  MCP tools.rs fetch_recent(agent-presence,500) + count.saturating_sub(slice_size)
  count-seek has the same T-2390/T-2391 staleness under latest_per_cv_key; MCP presence
  tools (presence_now/listeners/find_idle) read stale. Mirror the CLI cv_index fix
  (subscribe include_current_value) in termlink-mcp.

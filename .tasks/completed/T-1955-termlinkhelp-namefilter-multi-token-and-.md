...
description: >
  Split name_filter on whitespace into tokens; match tools whose name+description
  contains EVERY token (case-insensitive, any order). Single-token behavior unchanged.
  Closes the intent-search gap: LLMs querying 'send message' or 'agent post' get conjunctive
  results instead of empty/spurious matches. T-1954 follow-up.

...
`evaluate_presence_msgs` (MCP, tools.rs) and its one-to-one mirror
`evaluate_presence` (CLI, channel.rs) count a post toward `online` when
`ts >= cutoff`, with no `ts <= now_ms` upper bound. A future-dated envelope
keeps `online == true` forever, so `agent contact --require-online` passes for a
dead peer and an orchestrator dispatches work to a corpse. Same missing-upper-
clock-bound class as T-2536 (find_idle), T-2598 (agent-listeners.sh), and the
chat-arc-stats `ts > now_ms` drop (tools.rs:4029). The two helpers are
maintained one-to-one (the MCP docstring asserts it), so both get the guard in
this one task (PL-318 cross-copy audit).
...
## Verification
cargo test -p termlink-mcp evaluate_presence
cargo test -p termlink presence_
grep -q 'ts >= cutoff && ts <= now_ms' crates/termlink-mcp/src/tools.rs
grep -q 'ts >= cutoff && ts <= now_ms' crates/termlink-cli/src/commands/channel.rs
...
**Symptom:** `agent contact --require-online` (and any consumer of the presence
helpers) reports a dead peer as ONLINE when its last envelope is future-dated,
so an orchestrator dispatches work to a corpse.

**Root cause:** `if ts >= cutoff { posts_in_window += 1 }` has only a lower
bound. A future `ts` satisfies it forever → `online = posts_in_window > 0`.

**Why structurally allowed:** the future-clock guard exists elsewhere (T-2536,
tools.rs:4029) but not on these two presence helpers, which are maintained as
one-to-one mirrors — so the omission was duplicated and invisible.

**Prevention:** a future-ts unit test in each crate asserts `online == false`
(load-bearing, temp-revert proven). The fix adds `&& ts <= now_ms` to both.
...
### 2026-08-10T20:26:19Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/termlink/.tasks/active/T-2599-presence-eval---require-online-gate-coun.md

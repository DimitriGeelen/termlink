---
id: T-2462
name: "advance_cursor overwrites the persisted cursor instead of taking the max, so
  a stale or retried advance can regress a subscriber's delivery frontier and re-deliver
  already-consumed records — make it monotonic like the claim-ack path (round-16 F3)"
description: >
  advance_cursor overwrites the persisted cursor instead of taking the max, so a stale
  or retried advance can regress a subscriber's delivery frontier and re-deliver already-consumed
  records — make it monotonic like the claim-ack path (round-16 F3)

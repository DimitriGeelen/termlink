---
id: T-598
name: "Add --first flag to discover command that outputs just the first matching session
  ID"
description: >
  Add --first flag to discover command that outputs just the first matching session
  ID
...
### Agent
- [x] --first flag added to Discover command in cli.rs
- [x] main.rs passes first to cmd_discover
- [x] cmd_discover outputs just the first matching display name when --first is passed (exit 1 if no match)
...
### 2026-03-28T16:32:08Z — task-created [task-create-agent]

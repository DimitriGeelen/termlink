...
description: >
  Field-discovered in T-2350: with --to-session targeting a session on a REMOTE hub
  (tl-dzbcxxka on 192.168.10.122:9100), the doorbell inject ran against the LOCAL
  hub ('session missing?' WARN x2) — the mail posted but the doorbell never rang the
  peer. Workaround used: direct 'channel post --hub <peer-hub>' + 'termlink remote
  inject <hub> <session>'. Fix: agent-send.sh explicit-routing path must carry/accept
  the peer hub (--hub flag or resolve from presence) and use remote inject when the

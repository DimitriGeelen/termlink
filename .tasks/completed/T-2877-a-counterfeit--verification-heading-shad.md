...
horizon: null
tags: []
components: [scripts/check-verification-heading-shadow.sh, tests/verification-heading-shadow-fixtures.sh]
related_tasks: []
...
last_update: 2026-09-01T21:43:23Z
date_finished: 2026-09-01T21:43:23Z
...
- **AC#1 (Agent)** — **The defect is reproduced against the REAL extractor, not a re-typed copy.** The check must call the framework's own `extract_verification_block` (`.agentic-framework/lib/verification-port.sh`) rathe
  - **AC-verify-mismatch** (narrow, heuristic) — `path=agentic-framework/lib/verification-port.sh in: **The defect is reproduced against the REAL extractor, not a re-typed copy.** The check must call the framework's own `extract_verification_block` (`.`

### 2026-09-01T21:43:23Z — status-update [task-update-agent]

...
description: >
  Final field-rollout step: install hourly heartbeat cron on .121 so it actively contributes
  to agent-chat-arc like .107/.122/.141 do today. Blocked from agent-driven path because
  no termlink session is registered on .121 (no remote-exec channel). Once operator
  runs the install steps below + registers a session, future agents can drive .121
  directly.

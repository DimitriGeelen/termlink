...
### Agent
- [x] `crates/termlink-mcp/src/tools.rs` changes `#[tool_router]` to `#[tool_router(vis = "pub(crate)")]` with a comment block explaining WHY for future maintainers
- [x] `cargo clean && cargo build -p termlink-mcp` passes (rmcp-macros 1.3.0 path)
- [x] Fresh `cargo build -p termlink-mcp` with rmcp-macros **1.4.0** forced via `cargo update --precise 1.4.0` succeeds — this is the precise reproduction of `cargo install --git` without `--locked` (evidence: "Finished `dev` profile" with rmcp-macros v1.4.0 compiling in the build log)
- [x] `cargo test -p termlink --bin termlink` passes (189 tests, confirmed under BOTH rmcp-macros 1.3.0 and 1.4.0)
- [x] No clippy warnings (`cargo clippy -p termlink-mcp` clean)
- [x] G-019 post-fix root cause: register gap concern — `cargo install --git` (no `--locked`) is the documented external install path yet has no CI coverage. Root cause: CI builds use the lockfile (so rmcp-macros stays at 1.3.0) and never exercise the fresh-resolve path where the caret on rmcp's internal rmcp-macros dep allows 1.4+ to slip in.
...
### 2026-04-14T22:02:18Z — status-update [task-update-agent]

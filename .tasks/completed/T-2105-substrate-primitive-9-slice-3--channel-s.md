---
id: T-2105
name: "substrate primitive 9 slice 3 — channel subscribe --include-current-value CLI flag + MCP parity"
description: >
  substrate primitive 9 slice 3 — channel subscribe --include-current-value CLI flag + MCP parity
...
date_finished: 2026-06-09T21:18:41Z
...
# T-2105: substrate primitive 9 slice 3 — channel subscribe --include-current-value CLI flag + MCP parity

## Context

Substrate primitive #9 (broadcast-with-replay) — see T-2089 inception artifact
`docs/reports/T-2089-broadcast-with-replay-inception.md` (Design A: tagged-post
current-value via `metadata.cv_key`).

Slices 1+2 (T-2103/T-2104) shipped the RPC-tier foundation:
- Hub `cv_index: HashMap<topic, HashMap<cv_key, offset>>` recorded on every
  cv-tagged post (T-2103)
- `channel.subscribe` accepts `include_current_value: bool`, response gains
  `current_values: [{cv_key, offset, msg}, ...]` (T-2104)

Slice 3 surfaces this at the CLI + MCP tiers — the operator-visible layer.

Out of scope (later slices):
- Slice 4: `channel cv-keys [<TOPIC>]` operator inspection verb
- Slice 5: wire `metadata.cv_key=<agent_id>` into `scripts/listener-heartbeat.sh`
  (`/peers` becomes O(N_agents) instead of O(N_heartbeats) — the user payoff)
...
### 2026-06-09T21:00:41Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/termlink/.tasks/active/T-2105-substrate-primitive-9-slice-3--channel-s.md

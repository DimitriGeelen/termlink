---
id: T-2302
name: "V6-S5 journal-authoritative + firehose suppression for dm"
description: >
  V6-S5 journal-authoritative + firehose suppression for dm
...
components: []
related_tasks: [T-2291, T-2296, T-2298, T-2299, T-2300, T-2301]
...
date_finished: 2026-07-02T07:18:19Z
...
bvp_scores_proposed:
  - ts: '2026-08-18T18:56:38Z'
    ...
    rationale: D1=4-5 (body:new-class); D2=2 (body:telemetry-or-audit-entry); 
      D3=2 (body:default-change); D4=2 (body:env-class-handled); F-RECALL=3 
...
arc-003 reliable-comms V6 (apex, T-2296) **slice S5** — the LAST slice. Makes the
S1 per-conversation journal the **authoritative** durable record for `dm:` and takes
those durable messages OFF the hub firehose, so the firehose reverts to a short-window
store-and-forward buffer instead of an ever-growing log (the T-1991 bloat class).
Design: `docs/plans/T-2296-v6-direct-transport-first-design.md` **§2-S5**. Read that
first — these ACs are the acceptance surface, not a re-spec.

**Approach — option (b), client-side (design recommendation).** Chosen over option (a)
hub-side suppression (`channel.rs:512` routing dm: to a journal table) because (b) has
a **smaller blast radius, is reversible, and does not touch the hot post path**. The
journal (S1, `~/.termlink/journals/journal.sqlite`, populated by
`scripts/journal-mirror.sh`) becomes the read source; a **reaper** trims already-
journaled `dm:` turns/receipts from the firehose. Option (a) is the cleaner end-state
but is deferred (or behind a flag) — note it in Decisions.

**Prereqs already shipped:** S1 journal-mirror + `agent-journal.sh` (T-2298); S2/S3/S4
transport + sidecar + orchestration (T-2299/2300/2301). `/recent-dm` →
`scripts/recent-dm.sh` → the T-1849 recent-engine is the current firehose-reading read
path to re-point.

**Build-time subtleties to resolve (Evolution-worthy):**
1. **HOW to trim from the firehose.** Investigate whether the mechanism is
   `channel set-retention` on `dm:` topics + `channel sweep` (the T-2245 retention
   arc — bounded/latest-per-cv-key), a redaction, or a hub-side delete. Confirm which
   primitive actually removes turn/receipt envelopes from the bus store on the
   loopback hub before wiring the reaper.
2. **Message-loss safety (load-bearing).** A `dm:` envelope may ONLY be trimmed after
   its `(topic, offset)` row is durably present in `journal.sqlite`. Never trim ahead
   of the journal. Keep a store-and-forward window (recent un-journaled turns stay on
   the firehose) so an in-flight direct/fallback send is not reaped mid-delivery.
3. **Federation race (G-060 / [[reference_chatarc_dm_federation]]).** `dm:` topics
   federate with inbound sync lag. Trimming on the local hub must not run ahead of the
   inbound sync + journaling on the hubs that still need to pull the message. Scope the
   reaper to locally-authoritative offsets; document the multi-hub caveat.

**Scope boundary:** presence / broadcast / agent-chat-arc / store-and-forward topics
are UNTOUCHED — only `dm:` turn/receipt envelopes already in the journal are trimmed.
...
# (S5 commands — the reaper + its test file are created during the build.)
...
bash -n scripts/journal-reaper.sh
...
### 2026-07-01T21:53:14Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/termlink/.tasks/active/T-2302-v6-s5-journal-authoritative--firehose-su.md
- **Context:** Initial task creation

### 2026-07-02T07:18:19Z — status-update [task-update-agent]

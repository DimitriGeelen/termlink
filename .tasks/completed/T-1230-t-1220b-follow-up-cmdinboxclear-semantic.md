...
description: >
  Migrate inbox.clear callers (4 sites) with the Q4 semantic split: legacy inbox.clear
  deletes spool files on hub disk; channel-backed clear advances local cursor only
  (no hub mutation). Two competing semantics. **Inception-style task: design first,
  build later.**

...
description: >
  Implement per T-1209 GO: S1' spike first (test if non-zero SubagentStop exit mutates
  orchestrator-visible response); then bus-migration handler — over-threshold (T=8KB)
  returns auto-migrate to fw bus, orchestrator sees R-NNN pointer. Retires advisory
  check-dispatch.sh when live. Goal is no information loss. See docs/reports/T-1209-subagentstop-hook-inception.md.

---
id: T-1744
name: "termlink_hub_fingerprint + termlink_hub_export_secret MCP — rotation-protocol
  primitives (T-1166 MCP-parity)"
description: >
  MCP parity for the rotation-protocol primitive verbs documented in CLAUDE.md: hub_fingerprint
  (reads local hub.cert.pem, returns 12-char TLS fingerprint for peers to pin) and
  hub_export_secret (reads <runtime_dir>/hub.secret per R3 'read-live not cache' rule).
  Both are read-only co-located diagnostics — an MCP agent on the hub host can answer
  'what is my hub's fingerprint/secret?' without shelling out to the CLI.

...
description: >
  Add explicit documentation that channel topics are per-hub; cross-hub message visibility
  requires explicit --hub <addr> posting or remote_call channel.post. Topics with
  the same name on different hubs are independent state. Close the operator-facing
  portion of G-060.

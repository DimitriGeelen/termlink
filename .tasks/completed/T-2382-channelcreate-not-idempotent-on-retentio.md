---
id: T-2382
name: "channel.create not idempotent on retention — agent contact DM send aborts when
  dm topic exists with different retention (arc-004 fleet blocker)"
description: >
  agent contact posts a DM by first calling channel.create with Messages(1000); if
  the dm topic already exists as Forever the hub returns -32603 and the whole send
  aborts. Blocks arc-004 fleet DM delivery to any peer whose dm topic is Forever.
  Fix: make the create-before-post idempotent w.r.t. retention.
...
date_finished: 2026-07-09T09:10:01Z
...
- **Scan ID:** R-6af7c929
- **Timestamp:** 2026-07-09T09:19:40Z
- **Catalogue:** v1.3-seed
- **Overall:** CONCERN
- **Needs Human:** no
- **Findings:** 1
...
- **AC#1 (Agent)** — Root cause located: `ensure_topic()` at `crates/termlink-cli/src/commands/channel.rs:2386-2409` (Err arm propagated the -32603) + hub guard `crates/termlink-bus/src/meta.rs:38-46` (`TopicPolicyMismatc
  - **AC-verify-mismatch** (narrow, heuristic) — `path=crates/termlink-cli/src/commands/channel.rs in: Root cause located: `ensure_topic()` at `crates/termlink-cli/src/commands/channel.rs:2386-2409` (Err arm propagated the -32603) + hub guard `crates/te`

### 2026-07-09T09:10:01Z — status-update [task-update-agent]

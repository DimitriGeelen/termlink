---
id: T-2329
name: "Vendor policy/anti-patterns.yaml into consumer .agentic-framework — reviewer
  non-functional without it"
description: >
  The static-scan reviewer (fw reviewer) resolves its catalogue to framework_root/policy/anti-patterns.yaml
  then project_root/policy/anti-patterns.yaml (static_scan.py:2452-2457). Neither
  exists in /opt/termlink: the vendored .agentic-framework/ ships no policy/ dir.
  Result: every reviewer inline+dispatch here returned 'ERROR: catalogue not found'
  — the governance reviewer has been silently disabled for the whole project. Local
  repair applied 2026-07-03 (copied 514-line catalogue from upstream AEF into .agentic-framework/policy/,
  gitignored so it will vanish on next vendor refresh). Systemic fix: the AEF vendor/update
  process must include policy/ in consumer installs.
...
# T-2329: Vendor policy/anti-patterns.yaml into consumer .agentic-framework — reviewer non-functional without it
...
### 2026-07-03T09:31:15Z — task-created [task-create-agent]

...
### Human
- [x] [REVIEW] Smoke test: from a session NOT spawned by `register --shell` (so no env var inherited), run `termlink whoami` — should still resolve via PID-walk
  **Steps:**
...
cargo test -p termlink commands::metadata::tests --quiet 2>&1 | tail -8 | grep -qE "test result: ok"
...
### 2026-04-26T22:17:45Z — status-update [task-update-agent]

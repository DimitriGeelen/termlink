---
id: T-1897
name: "check-outbox.sh: refactor inline fp→status join to consume peer-presence-lookup.sh
  (close first-seen-wins bug + dedup)"
description: >
  check-outbox.sh: refactor inline fp→status join to consume peer-presence-lookup.sh
  (close first-seen-wins bug + dedup)

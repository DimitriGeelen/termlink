...
description: >
  arc-004 retrospective review finding #3 (LOW, CONFIRMED): S3 intercepts hub.ws_subscribe
  BEFORE process_request_message (server.rs ~883, ~899), so the per-sender rate governor
  never throttles it and rpc_audit::record never logs it. A client can spam ws_subscribe
  frames un-throttled (cheap — replaces a Vec) with no audit trail. Independent, bounded
  fix: route ws_subscribe through the same rate-limit + audit path as other authed
  RPCs (or add explicit governor.try_charge + audit record at the intercept site).

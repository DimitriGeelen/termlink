...
description: >
  Add essentials=true mode to termlink_help returning the first non-deprecated tool
  of each category as a flat list. Auto-derived from help_categories() registry order
  — drift-proof. Gives MCP clients a focused starter set (~27 tools out of 252) for
  cold-start learning without curating an essentials list manually.

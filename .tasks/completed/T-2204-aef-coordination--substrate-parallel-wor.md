...
First end-to-end consumer of the T-2018 §6 arc-parallel-substrate primitives shipped over the last several sessions (find-idle #2, claim #1, claim-transfer #3, release #1, governor #10, queue #5, cv_index #9). The substrate is built but has no in-tree consumer wiring the primitives into a working orchestrator policy. This task delivers that consumer in dry-run-first mode so it can be validated without an AEF worker present, and converts to live mode the moment an AEF worker arrives (find-idle watch wired at /tmp/aef-arrived.sh, pid in /tmp/aef-watch.pid).

Workload is the real backlog: agent-eligible tasks (owner!=human, horizon in {now,next}) in `.tasks/active/`. Coordination broadcast on agent-chat-arc at offsets 3186 (127.0.0.1), 721 (.121), 1333 (.122). .141 failed (chronic). Auto-greet wired on idle-state `new` events matching capability `backlog-drain|worker|aef` or agent_id substring `aef`.
...
# Orchestrator script exists and refuses live mode without explicit --live
test -x scripts/orchestrator-backlog-drain.sh
out=$(scripts/orchestrator-backlog-drain.sh --dry-run 2>&1); echo "$out" | grep -q "DISPATCH"
# Recipe doc references the script
test -f docs/operations/substrate-orchestrator-recipe.md && grep -q "orchestrator-backlog-drain" docs/operations/substrate-orchestrator-recipe.md
# Notify hook still present
test -x /tmp/aef-arrived.sh
...
### 2026-06-13T08:15:01Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/termlink/.tasks/active/T-2204-aef-coordination--substrate-parallel-wor.md

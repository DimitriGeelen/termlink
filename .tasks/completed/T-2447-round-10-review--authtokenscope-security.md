...
description: >
  Capture of round-10 adversarial auth/token/scope-enforcement review; build deferred
  (session at urgent budget). Decompose one-bug-one-task on pickup.
...
components: [crates/termlink-cli/src/commands/token.rs, 
      crates/termlink-hub/src/server.rs, crates/termlink-session/src/auth.rs]
related_tasks: []

...
description: >
  Patch fw task verify Python parser to strip HTML <!-- ... --> blocks from the Human
  AC region before counting checkboxes. Default template's example AC '[REVIEW] Dashboard
  renders correctly' currently surfaces as a real unchecked Human AC for every freshly-templated
  task, inflating the G-008 partial-complete signal with false positives. One-line
  re.sub fix in .agentic-framework/bin/fw line ~1981; mirror to upstream framework
  via Channel 1 (T-1192 dispatch pattern).

---
id: T-1284
name: "G-011 structural fix — fw doctor cache freshness check + own-hub live-read
  enforcement"
description: >
  Close G-011 (auth cache drift) by implementing the medium-term and long-term mitigations:
  (1) fw doctor compares ~/.termlink/secrets/<IP>.hex mtime/value against authoritative
  <runtime_dir>/hub.secret for self-hub profiles and warns on drift, (2) profiles
  using IP-keyed cache for self-hub read are deprecated with a migration hint to point
  secret_file directly at <runtime_dir>/hub.secret. Foundation for T-243 multi-turn

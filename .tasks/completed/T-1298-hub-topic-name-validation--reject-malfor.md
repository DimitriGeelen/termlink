...
description: >
  Discovered during T-1297 Spike 1: framework-agent on .107 has a live topic literally
  named "learning.shared</topic>\n<parameter name=\"from\">email-archive" — XML interpolation
  leaked into a topic string and the hub accepted it. Hub should reject malformed
  topic names at emit time (regex like ^[a-z0-9._:\-]+$ plus length cap). Risk: topic-string
  sprawl, indexing/routing surprises, log-injection via topic name. Independent of
  routing discipline — orthogonal failure class.

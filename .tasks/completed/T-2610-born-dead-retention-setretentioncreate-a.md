---
id: T-2610
name: "Born-dead retention: set_retention/create accept messages:0/days:0 (silent
  topic wipe on sweep)"
description: >
  Reliability #2: retention_from_json accepts value:0 -> Messages(0)/Days(0); on next
  sweep the topic is silently wiped (keep_last=0 / cutoff=now). set_retention returns
  ok:true; create silently defaults on invalid. Floor to >=1 + make create loud on
  present-but-invalid. Sibling of T-2604 born-dead ttl.

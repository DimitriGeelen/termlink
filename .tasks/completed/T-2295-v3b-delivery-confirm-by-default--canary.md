...
description: >
  RC3b fix. Flip T-2286 --await-ack to the DEFAULT for /agent-handoff, /reply, agent-send.sh;
  recipient sidecar (V3a) auto-acks, advancing the channel.receipts frontier. Add
  an unconfirmed-delivery canary: local sent-but-unconfirmed mirror made observable
  (kills the write-only-sink class, e.g. framework:pickup 36-sent/0-recv per G-063).
  ACs: default send confirms or fails LOUD (never silent 'sent'); recipient auto-ack
  wired; canary surfaces unconfirmed sends; /check-arc read emits a receipt.

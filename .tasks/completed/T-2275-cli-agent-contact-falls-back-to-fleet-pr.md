...
description: >
  T-2267 review item 4, slice 4 (findings 11,16). CLI cmd_agent_contact (agent.rs:804-850)
  resolves target name via local manager::find_session only (817); a peer on another
  hub is invisible -> 'Session not found'. Add a fleet-presence fallback resolving
  name->(hub,fp) via identity_fingerprint (T-2270 foundation), use the resolved hub
  for the dm post, and on not-found steer toward fleet presence rather than broadcast.
  Depends on T-2270.

...
description: >
  ...
  means capability-live not just code-merged. Gate point is AEF tooling (.agentic-framework/lib/arc.sh
  arc_close, human-invoked per G-062) — so this is a cross-repo change via the upstream-mirror
  pattern, NOT a pure termlink edit. Design: arc-closing task's ## Verification gains
  a one-line live-probe against the primary hub (fleet doctor / cv-keys capability
  probe). Bounded + reversible. Scope carefully: mandatory-block vs advisory-warn
  is a human call (IW-1).

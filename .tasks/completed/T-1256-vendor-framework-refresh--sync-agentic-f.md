...
# Version is pinned ≥ 0.9.1267 (the refresh target); auto-increments later, so use -ge.
python3 -c "import sys; v=open('/opt/termlink/.agentic-framework/VERSION').read().strip().split('.'); sys.exit(0 if tuple(map(int,v)) >= (0,9,1267) else 1)"
...
### 2026-04-25T16:10:49Z — status-update [task-update-agent]

...
description: >
  Every audit run prints the same three fabric WARNs (207/358 cards no edges; 10 unregistered
  files; 1 card outside watch patterns), each with a printed mitigation. Verify each
  mitigation actually clears its WARN; where it does not, identify and state why.
  Same class as T-2784 (a printed remedy that does not remedy).

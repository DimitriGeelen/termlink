---
id: T-1805
name: "Pickup-and-respond ritual — /check-arc reads turns AND posts receipt+reply
  (T-1800 build #2)"
description: >
  T-1800 build #2. Extend /check-arc (currently read-only) into the pickup-AND-respond
  ritual: when woken by an injected doorbell, read unread turns on dm:<self>:* topics,
  and for each, post a msg_type=receipt with the same conversation_id (so the sender's
  agent-send.sh T-1804 detects delivery) and a reply turn. Closes the 'respond' half
  of the doorbell+mail loop. Composes channel post/subscribe + ack; no protocol changes.

---
id: T-2243
name: "R4 substrate-fitness: dead-letter the silent poison-drop in the offline queue"
description: >
  R4 substrate-fitness: dead-letter the silent poison-drop in the offline queue
...
components: [crates/termlink-cli/src/commands/channel.rs, crates/termlink-session/src/bus_client.rs, crates/termlink-session/src/offline_queue.rs]
related_tasks: []
# arc_id:                         # T-1849: optional — slug (e.g. "arc-grooming") OR arc-NNN (e.g. "arc-005")
#                                 # When set, must resolve to .context/arcs/<id>.yaml; PreToolUse hook
#                                 # (check-arc-id) blocks save under agent control if it doesn't resolve.
#                                 # Empty/missing → unassigned (allowed). See CLAUDE.md §Task System.
created: 2026-06-22T11:47:58Z
last_update: 2026-06-22T19:43:03Z
date_finished: 2026-06-22T19:43:03Z
...
# T-2243: R4 substrate-fitness: dead-letter the silent poison-drop in the offline queue

## Context

Keystone of arc-substrate-fitness (T-2242 ingestion, gap #5 / F-INSTRUMENTATION). Source-confirmed:
`offline_queue.rs:223-225` `pop()` runs a bare `DELETE` on BOTH delivery and poison-drop, so a
discarded post — including a governance-plane "complete"/ledger message dropped during a hub blip —
vanishes with no trace. Replace the silent drop with a durable, readable dead-letter. The telemetry
transport (Q2/R5) will later ride this same durable queue. **Needs NO open Sovereign answer** —
safe to build once the human runs `fw arc start arc-substrate-fitness`.
Minted as the post-compact focus for continuity; confirm a fresh go before writing code (commit-cadence).
...
### 2026-06-22T11:47:58Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/termlink/.tasks/active/T-2243-r4-substrate-fitness-dead-letter-the-sil.md
- **Context:** Initial task creation

### 2026-06-22T19:43:03Z — status-update [task-update-agent]

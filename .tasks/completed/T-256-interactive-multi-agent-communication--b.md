...
description: >
  Inception: Should TermLink add an emit-to-target RPC so workers can push events
  directly
  to the orchestrator's event bus? Currently all event consumption is poll-based (collect/watch).
  Option B (collect fan-in) ships as T-257 with no code changes. This inception evaluates
  whether

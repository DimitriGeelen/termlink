...
description: >
  Enrich each name_filter match row with category_tool_count (size of the match's
  category) so an LLM ranking search results sees the namespace bound — a match in
  an 8-tool category is easier to learn than one in a 40-tool category. Composes with
  the existing {category, name, description, deprecated} shape. Drift-proof — derived
  live from help_categories().

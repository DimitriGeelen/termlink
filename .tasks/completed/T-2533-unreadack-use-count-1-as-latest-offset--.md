---
id: T-2533
name: "unread/ack use count-1 as latest offset — silently wrong after topic sweep
  (silent unread data-loss on core durable-messages verb)"
description: >
  count-1 != latest offset once a topic is swept (offsets monotonic via next_offset;
  count is COUNT(*)). Three sinks (channel.rs:3081 resolve_latest_offset, channel.rs:8697
  compute_unread_rows, tools.rs:2898 compute_unread_rows_mcp) use count-1 as the latest
  offset. After a sweep (normal path for agent-chat-arc, T-2252), channel inbox silently
  under-reports unread (drops rows where cursor>=stale-latest) and ack frontiers stick.
  Fix: add latest_offset (=next_offset-1) to bus (mirror oldest_offset meta.rs:231)
  + channel.list response + switch the 3 sinks. Agent-buildable, medium (5 files/4
  crates+test). Highest-severity campaign find.

---
id: T-2765
name: "Session-selftest INJECT + OUTPUT stages — close founding-verb proof gap (T-2694
  F1)"
description: >
  T-2694 F1: charter noun #3 asserts sessions are real PTYs — peers can stream output,
  inject keystrokes, exec, and doorbell-wake. session-selftest.sh (the prover for
  the founding verb) exercises only exec: 1 of 4 claims proven. inject's unit tests
  are named command_inject_resolves_keys_no_pty — they prove key-name resolution WITHOUT
  a PTY. Add real INJECT and OUTPUT stages so the prover covers what the charter claims.

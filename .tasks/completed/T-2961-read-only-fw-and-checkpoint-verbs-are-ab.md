...
description: >
  P-002's check-active-task gate blocks commands it cannot prove are read-only, and
  its own block message says so: 'this command writes nothing the gate can detect
  ... is not on the read-only allowlist (agents/context/lib/safe-commands.sh) ...
  If it genuinely only reads, that is a gap in the allowlist worth filing.' Measured
  this session: .agentic-framework/agents/context/checkpoint.sh status is blocked
  despite being a pure read of the session transcript that prints token usage. The
  consequence is specific and self-defeating: checkpoint.sh status is the verb the
  budget rule and the /resume skill both mandate for reading context budget, so an
  agent with no active task cannot check its own budget -- exactly the state where
  it most needs to, since having just closed a task is when it must decide whether
  to start another or hand over. Also observed: fw bvp estimate is blocked, though
  that one genuinely writes to the task file, so the gate is right to block it even

---
id: T-2428
name: "field-arm periodic retention sweeper on .107 hub"
description: >
  R2-GAP-B field application (T-2425/T-2427): rebuild termlink with the T-2426+T-2427
  code, install to /root/.cargo/bin, add TERMLINK_SWEEP_INTERVAL_SECS=3600 via systemd
  drop-in, restart hub THROUGH the unit (G-070), verify governor_status shows the
  sweeper armed and auth preserved (no re-pin).
...
# T-2428: field-arm periodic retention sweeper on .107 hub

## Context

Field application of T-2426 (debris auto-TTL) + T-2427 (env-gated periodic
sweeper) on the local .107 hub — same procedure class as T-2424 (build →
install → restart THROUGH the systemd unit per G-070, auth must survive via
persist-if-present).
...
- **Output:** /opt/termlink/.tasks/active/T-2428-field-arm-periodic-retention-sweeper-on-.md

---
id: T-2509
name: "termlink exec orphans child on timeout - executor::execute missing kill_on_drop"
description: >
...
# T-2509: termlink exec orphans child on timeout - executor::execute missing kill_on_drop

## Context

`termlink_session::executor::execute()` (the shared shell-exec helper behind the
charter's "control terminal sessions" verb) awaits `cmd.output()` under
`tokio::time::timeout` but never sets `kill_on_drop(true)`. On timeout the future
is dropped and tokio leaves the child running to natural completion — an orphaned
process. All four call sites pass a timeout and inherit the leak: CLI `termlink exec`
(`execution.rs:76`), session RPC handler (`handler.rs:412`), MCP exec
(`tools.rs:11643`), MCP batch (`tools.rs:14729`). The existing `execute_timeout`
test only asserts `Err(Timeout)` is returned; it never verifies the child was killed.
...
grep -q "kill_on_drop(true)" crates/termlink-session/src/executor.rs
...
### 2026-08-03T12:10:59Z — task-created [task-create-agent]

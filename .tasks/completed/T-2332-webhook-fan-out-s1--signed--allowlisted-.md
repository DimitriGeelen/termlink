...
- [x] `reqwest` (rustls-tls, no default features) + `hmac`/`sha2` added to `termlink-hub`; `cargo build -p termlink-hub` succeeds with no openssl/native-tls in the tree
- [x] `crates/termlink-hub/src/webhook.rs` exists and is registered in `lib.rs`, exposing `WebhookConfig`, `WebhookTarget`, `sign_payload`, `host_allowed`, `dispatch`
- [x] `sign_payload` produces `sha256=<hex>` HMAC-SHA256; a unit test asserts the canonical vector (key="key", "The quick brown fox…" → sha256=f7bc83f4…3cd8)
- [x] `host_allowed` is deny-by-default with EXACT host match (SSRF guard): unit tests cover empty-allowlist deny, exact allow, suffix-attack deny (`hooks.example.com.evil.com`), and garbage-URL deny
- [x] `dispatch` refuses a non-allowlisted target (e.g. `169.254.169.254` cloud-metadata) with `HostNotAllowed` BEFORE any network call — proven by an async unit test
- [x] `WebhookConfig::is_enabled()` is false with zero targets (opt-in: no configured targets ⇒ no-op, no behavior change); unit-tested + config parses from JSON
...
### 2026-07-03T09:50:44Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/termlink/.tasks/active/T-2332-webhook-fan-out-s1--signed--allowlisted-.md

...
# MISSING, UNINSTALLED_JOBS, or JOB_DRIFT — a dark canary is not a variation,
# and neither is an undeployed command change (T-2787).
...
uninst_n=${#uninstalled[@]}; ack_n=${#acknowledged[@]}; jobdrift_n=${#job_drift[@]}
...
if [ "$FORMAT" = json ]; then
    ...
    printf '{"ok":%s,"missing_count":%s,"uninstalled_jobs_count":%s,"job_drift_count":%s,"drift_count":%s,"acknowledged_count":%s,"ok_count":%s,"skipped_count":%s,"strict":%s,"lenient":%s,"missing":%s,"uninstalled_jobs":%s,"job_drift":%s,"drifted":%s,"acknowledged":%s}\n' \
        "$([ "$fire" = true ] && echo false || echo true)" \
        "$miss_n" "$uninst_n" "$jobdrift_n" "$drift_n" "$ack_n" "$ok_count" "$skip_n" \
        "$([ "$STRICT" -eq 1 ] && echo true || echo false)" \
        "$([ "$LENIENT" -eq 1 ] && echo true || echo false)" \
        "$(jarr "${missing[@]}")" "$(jarr "${uninstalled_detail[@]}")" \
        "$(jarr "${job_drift_detail[@]}")" "$(jarr "${drifted[@]}")" "$(jarr "${acknowledged[@]}")"

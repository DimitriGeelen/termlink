...
# Header
printf "%-22s %-7s %-10s %-18s %-7s %-7s %-9s %s\n" \
  "HUB" "STATE" "VERSION" "SESSION-FP" "/agent" "/check" "vendored" "LAST-ARC"
printf "%-22s %-7s %-10s %-18s %-7s %-7s %-9s %s\n" \
  "----------------------" "-------" "----------" "------------------" "-------" "-------" "---------" "--------"
...
"$LOCAL_BIN" fleet status 2>&1 | sed -E 's/\x1B\[[0-9;]*[a-zA-Z]//g' | \
  ...
  while read -r STATE HUB; do
    ...
    printf "%-22s %-7s %-10s %-18s %-7s %-7s %-9s %s\n" \
      "$HUB" "$STATE" "${VERSION:-?}" "$FP_SHORT" "$AGENT_HANDOFF" "$CHECK_ARC" "$VENDORED_HB" "$LAST"
  done

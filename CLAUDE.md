...
8. *(Optional, recommended for any non-trivial session)* Opt into agent-presence so peers can reach you: `/be-reachable` (T-1841). Idempotent — safe to run unconditionally; stops cleanly on `/be-reachable stop` before session end. Skip on throw-away sessions (<2 min) or hosts that should not appear on the fleet.

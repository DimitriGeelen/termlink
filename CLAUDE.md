...
| **Claim (CLAIM-WRITE)** | **`/claim <topic> [<unit-id>] [--owner <id>] [--ttl-secs N] [--json]`** | CLAIM-WRITE daily verb (T-2097) — reserves a unit of work on a topic by wrapping `termlink channel claim` (substrate primitive #1, T-2019). WRITE-side counterpart to T-2093 `/claims` — together complete the substrate #1 daily-verb surface across both directions. **This skill WRITES state.** Auto-mints `unit-id` (random 16-hex, T-2049 client-msg-id pattern) when omitted; auto-resolves `--owner` from `$TERMLINK_AGENT_ID` env then `~/.termlink/be-reachable.state` (T-1857 sender-resolution chain, T-1841 identity source). Refuses with hint when owner unresolved — never invents an owner (claim accountability invariant). **Loud refusal taxonomy** for known error classes: CLAIM_ALREADY_HELD → suggest `/claims <topic>` to see who; AUTH_FAIL → suggest `termlink fleet reauth` + `fleet doctor`; RATE_LIMITED (-32008) → suggest `/governor`; HUB_AT_CAPACITY (-32019) → `/governor`; unknown errors pass through verbatim. Success render shows topic + unit_id + auto-resolved owner (observable) + claim_id + lease TTL + next-step hints (release vs claim-transfer per T-2046). Don't auto-retry transient errors — surface and let operator decide. Natural orchestrator chain: `/find-idle --capability X` → `/claim` → `termlink channel claim-transfer` (T-2046 cooperative handoff). See `.claude/commands/claim.md`. |
| **Release (CLAIM-RELEASE)** | **`/release <claim-id> [--by <id>] [--json]`** | CLAIM-RELEASE daily verb (T-2098) — releases a claim by id by wrapping `termlink channel release` (substrate primitive #1, T-2019). RELEASE-side counterpart to T-2097 `/claim` — together complete the substrate #1 WRITE-side lifecycle (acquire → release) at the skill tier. **This skill WRITES state.** Auto-resolves `--by` identity via T-1857 sender-resolution chain (env → `~/.termlink/be-reachable.state` → refuse). Refuses with hint when identity unresolved — never invents an owner (the hub would refuse via CLAIM_NOT_OWNED -32017 anyway; refusing client-side surfaces the failure mode loudly). **Loud refusal taxonomy** for known error classes: CLAIM_NOT_FOUND → suggest `/claims <topic>` + `claims-history` (T-2074); CLAIM_NOT_OWNED → suggest `/claims <topic>` to see actual holder, `claim-transfer` (T-2046) cooperative path, or `claim-force-release` (Tier-0 last resort); AUTH_FAIL → `fleet reauth` + `fleet doctor`; RATE_LIMITED (-32008) → `/governor`; unknown errors pass through verbatim. Success render shows claim_id + topic + unit_id + auto-resolved identity (observable) + released_at. Don't auto-retry transient errors — surface and let operator decide. **Not in scope:** bulk release (`--all`/`--topic`), force release (Tier-0 only), auto-release on session end (leases expire naturally per T-2042). Natural lifecycle: `/claim` → do work → `/release`. See `.claude/commands/release.md`. |
